...
def get_wiki_page_object(request, slug, repo, page_name):
    ...
    try:
        dirent = seafile_api.get_dirent_by_path(repo.id, filepath)
        if dirent:
    ...
    file_url = get_service_url().strip() + reverse('wiki:slug', args=[slug, page_name])
